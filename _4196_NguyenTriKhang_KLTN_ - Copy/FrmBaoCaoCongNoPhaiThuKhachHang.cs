...
{
    public partial class FrmBaoCaoCongNoPhaiThuKhachHang : Form
    {
        public FrmBaoCaoCongNoPhaiThuKhachHang()
        {
            InitializeComponent();
        }

        private void FrmBaoCaoCongNoPhaiThuKhachHang_Load(object sender, EventArgs e)
        {
        ...
        public void LoadReport(DataTable dtReportData, DataTable dtHeaderData, DateTime ngayBD, DateTime ngayKT)
        {
            ...
            ReportDataSource rds = new ReportDataSource("dsBCCNPTKH", dtReportData);

            // Thiết lập ReportPath (Đảm bảo đường dẫn file .rdlc là chính xác)
            rptViewer.LocalReport.ReportPath = "rptBCCNPTKH.rdlc";
            ReportDataSource rdsHeader = new ReportDataSource("dsHeaderBCCNPTKH", dtHeaderData);
