...
namespace _4196_NguyenTriKhang_KLTN_
{
    public partial class PhieuXuatKho : Form
    {
        ...
        public PhieuXuatKho(string chucVu)
        {
            InitializeComponent();
            this.chucVu = chucVu;
        }
        ...
        {
            // TODO: This line of code loads data into the 'qLRBWDsPhieuXuatKho.CHUNGTU' table. You can move, or remove it, as needed.
            this.cHUNGTUTableAdapter.Fill(this.qLRBWDsPhieuXuatKho.CHUNGTU);
            LoadChungTu();
            LoadLoaiCT();
            SetColumnHeaders();
        ...
        private void LoadChungTu()
        {
            ...
            {
                string query = "SELECT * FROM CHUNGTU WHERE MACT LIKE 'PXK%'";
        ...
        private void LoadLoaiCT()
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                string queryTT = "SELECT * FROM LOAICHUNGTU WHERE TENCT = N'Phiếu xuất kho'";
        ...
        private void LoadTKNO()
        {
            ...
            string query = "SELECT TRIM(MaTK1) FROM TaiKhoanC1 WHERE MaTK1 = '632'";
        ...
        private string LayMaCTCuoiCung()
        {
            ...
            string connectionString = @"Data Source=LAPTOP-168ITQ3D;Initial Catalog=QLRBW;User ID=sa;Password=123;Encrypt=False;TrustServerCertificate=True";
            string query = "SELECT TOP 1 MaCT FROM CHUNGTU WHERE MaCT LIKE 'PXK%' ORDER BY MaCT DESC";
        ...
        private string TaoMaCTMoi()
        {
            ...
            string maCTMoi = "PXK" + soMoi.ToString("D3");
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            ...
            // ... (Code lấy các biến maCT, NgayLap, soCT, moTa, pTTT, maNV, maLCT, maBT)
            // 💡 Thêm biến maKH (Nếu Phiếu Xuất Kho không có KH, dùng DBNull.Value)
            string maKH = ""; // Giả định Phiếu Xuất Kho không có MaKH, hoặc lấy từ combobox
            ...
            {
                connection.Open();
                // SỬA LỖI 1: Bổ sung MaKH vào VALUES
                ...
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    ...
                    command.Parameters.AddWithValue("@Mota", moTa);
                    command.Parameters.AddWithValue("@PTTT", pTTT);
                    // THÊM THAM SỐ THIẾU: MaKH. Nếu không có MaKH, truyền DBNull.Value
                    ...
                    try
                    {
                        ...
                        // SỬA LỖI 2: CHỈ GỌI LƯU CHI TIẾT KHI LƯU CHỨNG TỪ THÀNH CÔNG
                        LuuChiTietChungTu(); // Chỉ chạy khi INSERT CHUNGTU thành công
                    ...
                    {
                        // Nếu INSERT CHUNGTU thất bại, sẽ không gọi LuuChiTietChungTu()
                        MessageBox.Show("Lỗi lưu chứng từ (bảng CHUNGTU): " + ex.Message);
                    }
                }
            }
        }
        ...
        public DataTable GetThongTinChungTu(string maChungTu)
        {
            string connectionString = @"Data Source=LAPTOP-168ITQ3D;Initial Catalog=QLRBW;User ID=sa;Password=123;Encrypt=False;TrustServerCertificate=True";
            string query = @"
                                SELECT CHUNGTU.MaCT, day(CHUNGTU.NgayLap) AS Ngay, month(CHUNGTU.NgayLap) AS Thang, year(CHUNGTU.NgayLap) AS Nam, Buttoan.MaBT, TKNO, TKCO, FORMAT(Buttoan.SoLuong*Buttoan.DonGia, '#,##0', 'vi-VN') AS SoTien, SoCT, Mota, HoTen
	                            FROM CHUNGTU
        ...
        private DataTable LayDuLieuPhieuXuatKhoDetail(string maCT)
        {
            ...
            // Truy vấn chi tiết sản phẩm, cần JOIN với bảng SANPHAM để lấy Tên SP
            string query = @"
        ...
        private void btnXuatFile_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(maCT))
            {
                MessageBox.Show("Vui lòng chọn một Phiếu Xuất Kho để in.", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            try
            {
                ...
                FrmBaoCaoPhieuXuatKho frmBaoCao = new FrmBaoCaoPhieuXuatKho();
                frmBaoCao.HienThiBaoCaoPhieuXuatKho(dtHeader, dtDetail);
