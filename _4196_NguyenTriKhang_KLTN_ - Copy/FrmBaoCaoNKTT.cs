...
{
    public partial class FrmBaoCaoNKTT : Form
    {
        public FrmBaoCaoNKTT()
        {
            InitializeComponent();
        }

        private void FrmBaoCaoNKTT_Load(object sender, EventArgs e)
        {
        ...
        public void LoadReport(DataTable dtReportData, DataTable dtHeaderData, DateTime ngayBD, DateTime ngayKT)
        {
            ...
            ReportDataSource rds = new ReportDataSource("dsSoNhatKyThuTien", dtReportData);

            // Thiết lập ReportPath (Đảm bảo đường dẫn file .rdlc là chính xác)
            rptViewer.LocalReport.ReportPath = "rptSoNhatKyThuTien.rdlc";
            ReportDataSource rdsHeader = new ReportDataSource("dsHeaderNKTT", dtHeaderData);
